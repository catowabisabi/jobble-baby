#!/bin/bash
# Dispatch cycle 437 — Task 388: Feeding Progression Navigator (RESEND - window fix)
...
sleep 1
tmux send-keys -t "$TARGET" "cat > /tmp/task_388.txt << 'TASK_EOF'
...
## Keywords
food_protein_tolerance_window, feeding_progression_navigator, weaning_stages, FPIES_correlation, allergen_introduction_timeline, growth_velocity_overlay
...
2. app/(tabs)/_layout.tsx — add Tabs.Screen entry: name='feeding-progression', icon=clipboard-food-outline
...
- Use SleepArchitectureScreen or RegulatoryFitnessScreen as structural reference
...
TASK_EOF" Enter
sleep 2
tmux send-keys -t "$TARGET" "cat /tmp/task_388.txt | opencode --prompt" Enter
echo "Dispatched cycle 437 — task 388"
