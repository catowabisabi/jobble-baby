#!/bin/bash
# Dispatch cycle 537 — Re-dispatch: Jest test coverage for OralMotor, MilkTransfer, Teething, BottleRefusal
...
# Send to Sisyphus via tmux set-buffer + paste-buffer
tmux set-buffer -t jobble-baby "$(cat /tmp/sisyphus_task.txt)"
tmux paste-buffer -t jobble-baby
echo "Dispatched: cycle 537 — Jest test coverage OralMotor/MilkTransfer/Teething/BottleRefusal"
