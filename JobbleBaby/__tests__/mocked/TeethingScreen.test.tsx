/**
 * D. 前端 Mocked 測試 — TeethingScreen
 *
 * Uses mocked AsyncStorage to test teething timeline tracking UI
 *
...
import TeethingScreen from '../../app/(tabs)/teething';
...
describe('TeethingScreen', () => {
  ...
  it('should call safeGetItem on mount to load profile', async () => {
    renderWithProviders(<TeethingScreen />);
    await waitFor(() => {
      expect(safeGetItem).toHaveBeenCalled();
    });
  });

  it('should render teething screen without crashing', async () => {
    const { toJSON } = renderWithProviders(<TeethingScreen />);
    await waitFor(() => {
      expect(safeGetItem).toHaveBeenCalled();
    });
  ...
  it('should not crash on button press', async () => {
    const { queryByText } = renderWithProviders(<TeethingScreen />);
    await waitFor(() => {
      expect(safeGetItem).toHaveBeenCalled();
    });
